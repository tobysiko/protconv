...
class PDBToImageConverter():
    def __init__(self, data_path, pdb_path, png_path, npy_path, csv_path, id_list, 
                 img_dim=None,   overwrite_files=False, selection="protein and heavy", 
                 pdb_suffix=".pqr", channels=["dist","coulomnb","seqindex"], 
                 output_format="png", save_channels=True, randomize_list=False, 
                 distmat_cutoff=51.2, invert_img=False, maxnatoms=20000,
                 is_pdbbind=True, sample_modes=False, save_sparse_array=False, triangular_matrix=False,max_seq_dist=256, 
                 verbosity=0, nprocs=1, first_chain=False):
        ...
        if self.channels[0] == '':self.channels=[]
    ...
    def pdb2coulomb(self, prot, distmat, mincut=-0.1, maxcut=0.1):# requires data from PQR file!
        n = len(prot)
        cmat = np.zeros((n,n))
        charges = prot.getCharges()
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                if i<j:
                    ...
                    f=1.0#138.935458
                    E_coul = f*(q_i*q_j) / (e_r*r_ij)
                    if E_coul != 0: E_coul = math.log(E_coul)
    ...
    def get_atom_sequence(self, prot, pocket):
        ...
        if "pocket" in self.channels:
            ...
            #print(pocres)
            d["pocket"] = np.array([1 if r in pocres else 0 for r in d["rnum"]], dtype=np.int8)
        else:
    ...
    def convert_pdbs_to_arrays(self, verbosity=0, show_pairplot=False, show_distplots=False, show_matrices=False, show_protein=False):
        ...
        for di in range(n):
            ...
            try:
                if self.verbosity>1: print("Parse protein:")
                prot = parser(p).select(self.selection, quiet=True)
                
                pocket = None
                if self.is_pdbbind:
                    if self.verbosity>1: print("Parse pocket:")
                    pocket = prody.parsePDB(pp).select(self.selection, quiet=False)
                    if self.verbosity>1: print("pocket atoms:",pocket.numAtoms())
            ...
            print_progress(di+1, n, prefix='converting structures:', suffix=' - %s - %i atoms - calculating: %s'%(dom,nat,"atom csv".ljust(12)), decimals=2, bar_length=20, prog_symbol='O')
            ...
            try:
                for chain in chains:
                    ...
                    if self.verbosity>1: print("residues:",unires)
                    for r in unires:
                        alts = atom_df[ (atom_df["rnum"]==r) & (atom_df["chid"]==chain) ].loc[:,"altloc"].unique()
                        if len(alts)>1:
                            if self.verbosity>1: print("altloc:",alts)
                        aa = atom_df[ (atom_df["rnum"]==r) & (atom_df["chid"]==chain) ].loc[:,"rseq"].unique()
                        ...
                        if self.verbosity>1: 
                            if alist != can:
                                ...
                                if 'OXT' in alist: print("Found terminal OXT")
                                ...
                                pass
                                #print("canonical=1")
                        atom_df.loc[ (atom_df["rnum"]==r) & (atom_df["chid"]==chain) ,"canonical"] = alist==can
                        #print(atom_df[ (atom_df["rnum"]==r) & (atom_df["chid"]==chain) ])
                
                csvname = os.path.join(self.data_path, self.csv_path, dom+"_atomsequence.csv")
                if self.verbosity>=1: print("Writing: ",csvname)
            ...
            for ch in self.channels:
                if self.overwrite_files or is_missing[ch]:
                    ...
                    if show_distplots:
                        ...
                        plt.xlabel("normalized values");plt.ylabel("counts");plt.title("matrix histogram: "+ch);plt.show()
            ...
            if len(matrices)>=3:
                if show_pairplot:
                    ...
                    tmpdf = pd.DataFrame(data={ch:matrices[ch].flatten() for ch in self.channels[:3]
                                            })
    ...
    def my_mats_p(self, atom_df, mincut=-0.1, maxcut=0.1):
        ...
        if self.verbosity>=1: 
            ...
            elapsed = end - start
            print ("Elapsed time after calculating matrix elements (s): %s"%str(elapsed))
        ...
        for k,v in results:
            ...
            if parms["do_pocket"]: distmat_pocket[i,j] = distmat_pocket[j,i] = pmat
    ...
    def my_mats(self, atom_df, mincut=-0.1, maxcut=0.1):
        ...
        for a1 in range(n):
            xyz1 = np.array((atom_df.loc[a1, "x"], atom_df.loc[a1, "y"], atom_df.loc[a1, "z"]))#atomseq["coords"][a1]
            q1 = atom_df.loc[a1, "charges"]
            r1 = atom_df.loc[a1, "rnum"]
            for a2 in range(n):
                if a1<a2:
                    xyz2 = np.array((atom_df.loc[a2, "x"], atom_df.loc[a2, "y"], atom_df.loc[a2, "z"]))#atomseq["coords"][a2]
                    ...
                    seqdist[a1,a2]=seqdist[a2,a1]=min(self.max_seq_dist, abs(r1-r2))
                    seqindex[a1,a2]=seqindex[a2,a1]=min(r1, r2)%2 * min(self.max_seq_dist, abs(r1-r2))
                    if do_pocket:
                        distmat_pocket[a1,a2] = distmat_pocket[a2,a1] = atom_df.loc[a1, "pocket"] and  atom_df.loc[a2, "pocket"]
                elif a1==a2:
...
def pdb2pqr(idlist, pdbloc,pqrloc,exe="pdb2pqr",show_output=False, show_error=False, 
    ...
    for di in range(len(idlist)):
        ...
        if os.path.exists(dompdb) and (not os.path.exists(dompqr) or overwrite):
            cmd_string = "%s %s --ff=%s %s %s %s"%(exe, options, ff, ligand, dompdb, dompqr)
            print (cmd_string)
...
if __name__=="__main__":
    ...
    ap.add_argument("--selection", default="protein and aminoacid and heavy and not hetero and not nucleic and not water", help="a valid ProDy selection")
    ...
    ap.add_argument("--nprocs", type=int, default=1, help="Number of processes to use in parallel. Set to -1 to use all cores minus 2.")
    ...
    if args.calcpqr:
        ...
        pdb2pqr(idlist, os.path.join(args.basepath, args.pdbpath), os.path.join(args.basepath, args.pqrpath), exe=args.pdb2pqr, is_pdbbind=args.pdbbind, show_output=True, show_error=True, quiet=False, overwrite=args.overwrite)
    ...
    converter.convert_pdbs_to_arrays(show_pairplot=False, show_distplots=args.show_plots, show_matrices=args.show_plots, show_protein=args.show_plots, verbosity=args.verbosity)
