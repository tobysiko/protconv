...
class CATH_Classifier:
    ...
    def __init__(
        ...
        epochs=10,
        model="lenet",
        data_labels_filename="cath-dataset-nonredundant-S40.txt",
        ...
        use_img_decoder=False,
        dc_dec_weight=1.0,
        dc_dec_act="relu",
        dc_decoder_loss="mean_squared_logarithmic_error",
        use_seq_encoder=False,
        use_seq_decoder=False,
        seq_decoder_loss="mean_squared_logarithmic_error",
        seq_dec_weight=1.0,
        seq_code_layer="fc_first",
        seq_dec_act="relu",
        manual_seq_maxlength=None,
        seq_enc_arch="cnn",
        seq_dec_arch="cnn",
        lstm_enc_length=128,
        lstm_dec_length=128,
        ...
        cath_domlength_weight=1.0,
        domlength_regression_loss="mean_squared_logarithmic_error",
        use_biovec=False,
        ...
        biovec_flatten=True,
        biovec_ngram=3,
        generate_protvec_from_seqdict=True,
        seq_encoding="index",
        seq_padding="post",
        conv_filter_1d=3,
        conv_filter_2d=(3, 3),
        conv_filter_3d=(3, 3, 3),
        crops_per_image=0,
        crop_width=32,
        crop_height=32,
        no_classifier=False,
        fullconv_img_clf_length=512,
        fullconv_seq_clf_length=512,
        valsplit_seed=None,
        use_lstm_attention=False,
        use_embedding=False,
        classifier="dense",
        input_fasta_file="cath_pdb_to_seq.fasta",
        dataset="cath",
        merge_type="concatenate",
        conv_dropout=0.2,
        pad_value=0,
        blank_img_frac=0.0,
        seq_index_extra_dim=False,
    ):
        ...
        self.model = model
        assert self.model in self.ALLOWED_MODELS, (
            "Model must be one of: %s" % (str(self.ALLOWED_MODELS)) + self.model
        )
        ...
        if self.model_arch_file != None and self.model_weights_file != None:
            ...
            self.modlabel = os.path.split(self.model_arch_file)[-1].rename(
                "_model_arch.json", ""
            )
        self.X = None
        self.Ys = None
    ...
    def import_cath_dataset(self, img_h5_file=None, metadata_file=None):
        if img_h5_file != None:
            ...
            if self.img_dim != self.X.shape[1]:
                print(
                    "Rescaling images from %i to %i" % (self.X.shape[1], self.img_dim)
                )
                self.rescale_images()
    ...
    def export_cath_dataset(
        self, expath, name, export_images=True, export_metadata=True
    ):
        saved_files = []
        if export_metadata:
            df = pd.DataFrame()
        ...
        if export_images:
            hdf5_path = os.path.join(
                expath,
                "CATH_%s_%sx%sx%s_n%i.hdf5"
                % (name, self.img_dim, self.img_dim, self.nchannels, len(self.labels)),
            )
    ...
    def getTrainTestSetMulti_h5(self):
        ...
        if self.h5_backend == "h5py":
            ...
            if self.img_dim != None and self.img_size_bins == []:
                ...
                assert len(h5_input_list) == len(self.img_size_bins), "%i vs %i" % (
                    len(h5_input_list),
                    len(self.img_size_bins),
                )
                self.dX = {}
                self.dYs = {}
                self.dlabels = {}
    ...
    def getTrainTestSetMulti(
        self, resized_img_loc="", files=[], propose_image_bins=False
    ):
        if self.img_dim == None:
            self.save_resized_images = False

        if resized_img_loc == "":
            resized_img_loc = "%s%sx%s" % (
                self.image_folder,
                self.img_dim,
                str(self.img_dim),
            )
        ...
        with open(self.data_labels_filename) as f:
            for l in f:
                if self.data_labels_filename in [
                    os.path.join(self.root_path, x) for x in ["cath-domain-list.txt"]
                ]:
                    ...
                    if line[0] != "#":
                        ...
                        if self.data_labels_filename in [
                            os.path.join(self.root_path, x)
                            for x in ["cath-domain-list.txt"]
                        ]:
                ...
                try:
                    ...
                    if self.nchannels == 3:
                        ...
                        if type(img) == type(None):
                            ...
                            continue
                        orig_size = img.shape[1]
                        if self.min_img_size != -1 and orig_size < self.min_img_size:
                            continue
                        if self.max_img_size != -1 and orig_size > self.max_img_size:
                            continue
                        # print (img.shape)
                    ...
                    if self.nchannels == 1:
                        ...
                        if K.image_data_format() == "channels_first":
                            x = np.reshape(
                                np.array(img), (3, self.img_dim, self.img_dim)
                            )
                    ...
                    if self.save_resized_images:
                        ...
                        if self.nchannels == 1:
                            tmp = np.reshape(x, (self.img_dim, self.img_dim))
                        else:
                            if K.image_data_format() == "channels_last":
                                tmp = np.reshape(
                                    x, (self.img_dim, self.img_dim, self.nchannels)
                                )
                            else:
                                tmp = x
                        # print(x.shape)
                        cv2.imwrite(
                            os.path.join(resized_img_loc, os.path.basename(f)) + ".png",
                            tmp,
                        )
                    ...
                    if self.inverse_img:
                        for i in range(self.nchannels):
                            if K.image_data_format() == "channels_last":
                                matrix = x[:, :, i]
                                x[:, :, i] = 256 + (
                                    -255 * (matrix - np.amin(matrix)) / np.amax(matrix)
                                )
                            else:
                                matrix = x[i, :, :]
                                x[i, :, :] = 256 + (
                        ...
                        for L in self.label_columns:
                            if L in [2, 3, 4, 5]:
                                pred_classes.append(
                                    ".".join(
                                        [
                                            dom_dict[cath_code][j - 2]
                                            for j in range(self.label_columns[0], L + 1)
                                        ]
                                    )
                                )
                            elif L in [
                    ...
                    if self.verbose:
                        print(
                            cath_code, pred_classes
                        )  # , [class_dict[p] for p in pred_classes[:3]])

                    if self.img_dim != None and self.img_size_bins == []:
                        if x.shape[1] != self.img_dim:
                            print(
                                "Unexpected image size %s! Expected %s"
                                % (x.shape, self.img_dim)
                            )
                            continue
                        ...
                        if old_dim <= new_dim:
                            bg = np.zeros(
                                (new_dim, new_dim, self.nchannels), dtype=x.dtype
                            )
                            if K.image_data_format() == "channels_first":
        ...
        if propose_image_bins:
            if self.img_dim != None and self.img_size_bins == []:
                ...
                print(
                    "\n Propose %i k-means clusters over img size:"
                    % (self.img_bin_clusters)
                )
                ...
                for l in range(len(cl)):
                    ...
                    plt.plot(
                        ...
                        [20, 20],
                        marker="o",
                        label="cluster_%i: #%i (%i--%.2f--%i)"
                        % (cl[l], cl_size, cl_min, cl_mean, cl_max),
                    )
                plt.legend(loc=0)
                ...
                print(
                    "To use these bins: '--dim 0 --img_bins %s'"
                    % (",".join(list(map(str, bins))))
                )

                print(
                    "\n Propose %i equally sized image-size bins:"
                    % (self.img_bin_clusters)
                )
    ...
    def prepare_dataset(self, show_classnames=False):
        ...
        if self.img_size_bins == []:
            ...
            total_Y_mat = [
                pd.get_dummies(total_Ys[y]).values
                for y in range(len(self.label_columns))
            ]
            ...
            for si in range(len(self.img_size_bins)):
                ...
                for y in range(len(self.label_columns)):
                    ...
                    self.dYmats[s][y] = total_Y_mat[y][
                        sum(tmp_sn[: si + 1]) : sum(tmp_sn[: si + 2])
                    ]
        ...
        if os.path.exists(loc):
            try:
                for altloc in ["ABCDEFG"]:
                    sequence = (
                        prody.parsePDB(loc, altloc=altloc)
                        .select("protein and calpha")
                        .getSequence()
                    )
    ...
    def get_sequence_data(self, save_fasta=False):
        if self.seq_from_pdb:
            ...
            if False:
                with open(
                    os.path.join(self.root_path, "cath-domain-seqs.fa")
                ) as fastafile:
                ...
                with open(
                    os.path.join(self.root_path, "seqdict_20798.fa")
                ) as fastafile:
                    for entry in fastafile.read().split(">")[1:]:
                        try:
                            tmp = entry.split("\n")
    ...
    def prepare_sequence_data(self):
        ...
        if self.use_biovec:
                ...
                for ci in range(len(self.labels)):
                    ...
                    if cath_code in self.seqdict:
                        ...
                        if type(seq) != type(None):
                            ...
                            print(
                                "    SOLVED: obtained sequence from PDB file for ",
                                cath_code,
                            )
                        else:
            ...
            if self.seq_encoding == "index" and self.seq_index_extra_dim:
                self.seqdata = self.seqdata.reshape(
                    (len(self.seqdata), input_seqlength, 1)
                )
    ...
    def load_model_from_files(self, modlabel_abspath):
        ...
        for i in input_shapes:
            if "seq_input" in i:
                ...
                if sdim == 7:
                    self.seq_encoding = "physchem"
                elif sdim > 7:
                    self.seq_encoding = "onehot"
                else:
                    self.seq_encoding = "index"
    ...
    def aa2phys(self, aa_1_letter):
        return np.array(
            [
                self.aa2phys_dict[aa_1_letter]["hp"],
    ...
    def sequence_encoding(self, num_words=21):
        ...
        if self.seq_encoding == "index":
            ...
            for seq in self.sequences:
                ...
                tmparr[:, 0] = 1.0
                tmparr[0 : min(len(seq), input_seqlength),] = np.array(
                    [aa2num[seq[l]] for l in range(min(len(seq), input_seqlength))]
                )
                # print(seq)
                # for row in tmparr:
            ...
            for seq in self.sequences:
                tmparr = np.array(
                    [
                        self.aa2phys(seq[l])
                        for l in range(min(len(seq), input_seqlength))
                    ]
                )
                seqdata.append(tmparr)
        ...
        seqdata = sequence.pad_sequences(
            seqdata,
    ...
    def randomly_blank_images(self, fraction=0.1):
        if fraction > 0.0:
            ...
            blank_ids = random.sample(
                range(len(self.labels)), int(len(self.labels) * fraction)
            )
    ...
    def biovec_encoding(self):
        if self.generate_protvec_from_seqdict:
            ...
            self.pv = biovec.ProtVec(
                fasta_fname=os.path.join(
                    self.root_path, self.input_fasta_file
                ),  # fasta file for corpus
                ...
                corpus_fname=os.path.join(
                    self.root_path, self.input_fasta_file.split(".")[0] + "_corpus.txt"
                ),
                sg=1,
                window=25,
            ...
            self.pv.save(
                os.path.join(
                    self.root_path,
                    self.input_fasta_file.split(".")[0] + "_protvec.model",
                )
            )
            self.biovec_model = os.path.join(
                self.root_path, self.input_fasta_file.split(".")[0] + "_protvec.model"
            )
        ...
        for seq in self.sequences:
            ...
            if not all(aa_valididity):
                if "B" in seq:
                    seq = seq.replace("B", "X")  # TODO look into this
                if "X" in seq:
                    seq = seq.replace("X", "A")  # TODO look into this
                aa_valididity = [x in "ACDEFGHIKLMNPQRSTVWYX" for x in seq]
                assert all(aa_valididity), " : \n%s\n%s" % (
                    seq,
                    "".join(map(str, map(np.int8, aa_valididity))),
                )
        ...
        if len(self.seqdict) != 0:
            ...
            if outpath == "":
                outpath = os.path.join(
                    self.root_path, "seqdict_%i.fa" % (len(self.seqdict))
                )
    ...
    def get_model(self, generate_modlabel=True, plot_model_diagram=True):
        ...
        if not self.load_model:
            if self.model == "fullconv":
                my_model = self.get_fullconv_model(
                    branch_2d=self.use_img_encoder or self.use_img_decoder,
                    branch_1d=self.use_seq_decoder or self.use_seq_encoder,
                )
                ...
                if K.image_data_format() == "channels_last":
                    input_tensor = Input(
                        shape=(self.img_dim, self.img_dim, self.nchannels)
                    )
                else:
                    input_tensor = Input(
                        shape=(self.nchannels, self.img_dim, self.img_dim)
                    )
    ...
    def generate_modlabel(self):
        ...
        if self.model == "fullconv":
            if self.use_img_encoder or self.use_img_decoder:
                ...
                if self.use_img_decoder:
                    arch_string += "D%.2f" % (self.dc_dec_weight)
        ...
        clf_reg_string = "clf"
        if "cath01_class" in self.nb_classes:
            clf_reg_string += "C"
        if "cath02_architecture" in self.nb_classes:
            clf_reg_string += "A"
        if "cath03_topology" in self.nb_classes:
            clf_reg_string += "T"
        if "cath04_homologous_superfamily" in self.nb_classes:
            clf_reg_string += "H"
        if "sequence_length" in self.nb_classes:
            clf_reg_string += "L"

        self.modlabel = (
            "%s_%s%s_%s_Dr%s_%s_pt%i_%sx%sx%i_%i_bt%i_ep%i_f%ix%i_fc%ix%i_%s_%iG"
            % (
                self.generic_label,
                self.model,
                arch_string,
    ...
    def dense_block(
        ...
        if act == "default":
            act = self.act
        if names == False:
            names = [False for _ in range(n_layers)]
        ...
        for i in range(n_layers):
            ...
            x = Dense(
                n_neurons, kernel_initializer=init, name=names[i] + "dense" + str(i)
            )(x)
        ...
        for i in range(n_layers):
            ...
            if dim == 1:
                x = Conv1D(
                    n_filters,
                    self.conv_filter_1d
                    if type(filter_shape) == type(None)
                    else filter_shape,
                    padding="same",
    ...
    def attention_3d_block(self, inputs, seq_length, SINGLE_ATTENTION_VECTOR=False):
        ...
        a = Reshape((input_dim, seq_length))(
            a
    ...
    def seq_input(self, input_seqlength):
        ...
        if self.use_biovec:
            if self.biovec_flatten:
                seq_input = Input(
                    shape=(self.biovec_length * 3, 1), dtype="float32", name="seq_input"
                )
            else:
                seq_input = Input(
                    shape=(self.biovec_length, 3), dtype="float32", name="seq_input"
                )
        else:
            if self.seq_encoding == "onehot":
                seq_input = Input(
                    shape=(input_seqlength, 22), dtype="float32", name="seq_input"
                )
            elif self.seq_encoding == "physchem":
                seq_input = Input(
                    shape=(input_seqlength, 7), dtype="float32", name="seq_input"
                )
            else:
                if self.seq_enc_arch in ["cnn"] or not self.use_embedding:
                    seq_input = Input(
                        shape=(input_seqlength, 1), dtype="float32", name="seq_input"
                    )
                else:
                    seq_input = Input(
                        shape=(input_seqlength,), dtype="float32", name="seq_input"
                    )
        return seq_input

    def conv_block_img_encoder(
        self, input, img_filters, n_layers, spatial_dropout=True
    ):
        conv = self.conv_block(
            ...
            act=self.act,
            init="he_normal",
            layer_order=self.batch_norm_order,
            filter_shape=self.conv_filter_2d,
        )
        if spatial_dropout:
            conv = SpatialDropout2D(self.dropout)(conv)

        return conv

    def conv_block_seq_encoder(
        self, input, seq_filters, n_layers, spatial_dropout=True, small_filter=False
    ):
        conv = self.conv_block(
            input,
            1,
            ...
            init="he_normal",
            layer_order=self.batch_norm_order,
            filter_shape=self.conv_filter_1d,
        )
        if spatial_dropout:
            conv = SpatialDropout1D(self.dropout)(conv)
        if small_filter:
            ...
            if spatial_dropout:
                conv3 = SpatialDropout1D(self.dropout)(conv3)
    ...
    def conv_img_unet_encoder(self, input, img_start_filters):
        ...
        conv5 = self.conv_block(
            pool4,
            2,
            img_start_filters * 16,
            n_layers=2,
            act=act_img_enc,
            init="he_normal",
            layer_order=self.batch_norm_order,
            names=[0, "img_conv_bottleneck"],
        )
        drop5 = Dropout(self.dropout)(conv5)

        return drop5, drop4, conv3, conv2, conv1

    def conv_img_unet_decoder(
        self, drop5, drop4, conv3, conv2, conv1, img_start_filters
    ):
        ...
        up6 = Conv2D(
            ...
            activation=act_img_dec,
        )(up6)
        merge6 = concatenate([drop4, up6], axis=3)
        conv6 = self.conv_block(
            merge6,
            2,
            img_start_filters * 8,
            n_layers=2,
        ...
        img_activation_out = Activation("linear", name="img_decoder")(
            Conv2D(self.nchannels, 1)(conv9)
        )
    ...
    def conv_seq_unet_encoder(self, seq_input, seq_start_filters):
        ...
        seq_conv5 = self.conv_block(
            seq_pool4,
            1,
            seq_start_filters * 16,
            n_layers=2,
            act=act_seq_enc,
            init="he_normal",
            layer_order=self.batch_norm_order,
    ...
    def conv_seq_unet_decoder(
        self, seq_drop5, seq_drop4, seq_conv3, seq_conv2, seq_conv1, seq_start_filters
    ):
        ...
        seq_up6 = Conv1D(
            ...
            padding="same",
            kernel_initializer="he_normal",
            activation=act_seq_dec,
        )(
            seq_up6
        )  # 200 / 12
        seq_merge6 = concatenate([seq_drop4, seq_up6], axis=2)
        seq_conv6 = self.conv_block(
            seq_merge6,
            1,
            seq_start_filters * 8,
            n_layers=2,
        ...
        if self.use_biovec:
            if self.biovec_flatten:
                seq_activation_out = Activation("linear", name="seq_dec")(
                    Conv1D(1, 1, activation="linear")(seq_conv9)
                )
            else:
                seq_activation_out = Activation("linear", name="seq_dec")(
                    Conv1D(3, 1, activation="linear")(seq_conv9)
                )
        else:
            if self.seq_encoding == "index":
                seq_activation_out = Activation("linear", name="seq_dec")(
                ...
                seq_activation_out = Activation(
                    self.softMaxAxis(2), name="seq_decoder"
                )(seq_conv10)
    ...
    def rec_seq_decoder(self, seq_drop5):  # TODO: unfinished
        ...
        seq_activation_out = LSTM(
            self.lstm_dec_length,
            activation=self.act,
            return_sequences=True,
            name="seq_decoder",
        )(seq_drop5)
    ...
    def seq_fullconv_clf_linker(self, seq_drop5):
        ...
        seq_conv_down = self.conv_block(
            seq_pool_down,
            1,
            self.fullconv_seq_clf_length,
            n_layers=2,
            act=self.act,
            init="he_normal",
            layer_order=self.batch_norm_order,
    ...
    def get_seq2img_model(self):
        ...
        lstm_out, lstm_out2 = Bidirectional(
            CuDNNLSTM(
                self.lstm_enc_length,
                kernel_initializer="he_normal",
                recurrent_initializer="orthogonal",
        ...
        img_activation_out = self.conv_img_unet_decoder(
            drop5, drop4, conv3, conv2, conv1, 64
        )
        ...
        outputs = self.add_cath_outputs_to_model(
            None if self.no_classifier else drop5,
            img_decoder=img_activation_out,
            seq_decoder=None,
        )
    ...
    def get_seq2img_model2(self):
        ...
        lstm_out = Dense(
            int((self.img_dim * self.img_dim * self.nchannels) / input_seqlength)
        )(lstm_out)
        lstm_out = BatchNormalization()(lstm_out)
    ...
    def get_generator(self, compile=True, add_outputs=False):
        ...
        with open(
            os.path.join("/hpc/ai_ml/ELT/results", unet_name + "_model_arch.json")
        ) as arch:
            unet = model_from_json(arch.read())
        unet.load_weights(
            os.path.join("/hpc/ai_ml/ELT/results", unet_name + "_model_weights.hdf5")
        )
        ...
        # print( unet.summary() )
        if add_outputs:
            self.add_cath_outputs_to_model(
                None
                if self.no_classifier
                else unet.get_layer(name="img_conv_bottleneck").output,
                img_decoder=None,
                seq_decoder=None,
            )

        self.generator = Model(
            seq_input, unet(merge), name="generator_G"
        )  # Model(seq_input, unet.output)
        print(self.generator.summary())
        print(self.generator.get_layer(name="model_4").summary())

        if compile:
            self.generator.compile(
                loss="mean_absolute_error", optimizer=self.get_optimizer()
            )
    ...
    def get_discriminator(self):
        ...
        self.discriminator.compile(
            loss="binary_crossentropy", optimizer=self.get_optimizer()
        )
        plot_model(
            self.discriminator,
            to_file=os.path.join(
                self.outdir, self.modlabel + "_discriminator-diagram.png"
            ),
            show_shapes=True,
    ...
    def get_discriminator_from_G(self, G):
        ...
        self.discriminator = Model(
            G.get_layer(name="model_4")(input), output, name="discriminator_D"
        )
    ...
    def train_gan(self):
        ...
        train_img, val_img, train_seq, val_seq = train_test_split(
            self.X, self.seqdata, test_size=self.valsplit
        )
        ...
        for e in range(1, self.epochs + 1):
            ...
            for b in range(int(batch_count)):
                ...
                image_batch = train_img[
                    np.random.randint(0, train_img.shape[0], size=self.batch_size)
                ]
                seq_batch = train_seq[
                    np.random.randint(0, train_seq.shape[0], size=self.batch_size)
                ]
                ...
                print(
                    "batch %i/%i: d_loss_real = %f --- d_loss_fake = %f --- g_loss = %f"
                    % (b + 1, int(batch_count), d_loss_real, d_loss_fake, g_loss)
                )
            ...
            if e == 1 or e % 20 == 0:
                self.plot_generated_images(
                    e, self.generator, indices=[1, 10, 100, 1000]
                )
        ...
        self.modlabel += "_split%.2f_lo%.3f_vlo%.3f" % (
            self.valsplit,
        ...
        self.model_arch_file = os.path.join(
            self.outdir, "%s_model_arch.json" % self.modlabel
        )
        self.model_weights_file = os.path.join(
            self.outdir, "%s_model_weights.hdf5" % self.modlabel
        )
        ...
        if self.ngpus == 1:
            ...
            plot_model(
                self.keras_model,
                to_file=os.path.join(self.outdir, self.modlabel + "_model-diagram.png"),
                show_shapes=True,
                show_layer_names=True,
                rankdir="TB",
            )  #'LR'
    ...
    def plot_generated_images(self, epoch, generator, indices=[1, 10, 100, 1000]):
        ...
        test_imgs = [
            np.squeeze(
                generator.predict(
                    self.seqdata[i,].reshape([1] + list(self.seqdata.shape[1:]))
                )
            )
            for i in indices
        ]
        ...
        axes[0, 0].imshow(test_imgs[0], interpolation="none")
        axes[0, 0].set_title(titles[0])
        axes[0, 1].imshow(test_imgs[1], interpolation="none")
        axes[0, 1].set_title(titles[1])
        axes[1, 0].imshow(test_imgs[2], interpolation="none")
        axes[1, 0].set_title(titles[2])
        axes[1, 1].imshow(test_imgs[3], interpolation="none")
        ...
        plt.savefig(
            os.path.join(
                self.outdir, self.modlabel + "_generated_images_ep%i.png" % epoch
            )
        )
        plt.close("all")
    ...
    def get_fullconv_model(self, branch_2d=True, branch_1d=True):
        ...
        if branch_2d:
            ...
            if self.use_img_encoder:
                ...
                input = Input(input_size, dtype="float32", name="main_input")  # 224
                inputs.append(input)
                drop5, drop4, conv3, conv2, conv1 = self.conv_img_unet_encoder(
                    input, img_start_filters
                )
                if not self.fullconv_img_clf_length in [0, None]:
                    final_2d = self.img_fullconv_clf_linker(drop5)
                    final_pool = GlobalAveragePooling2D(name="final_2d")(
                        final_2d
                    )  # latent space vector
                else:
                    final_2d = drop5
            ...
            if self.use_img_decoder:
                img_activation_out = self.conv_img_unet_decoder(
                    drop5, drop4, conv3, conv2, conv1, img_start_filters
                )
                # img_activation_out.name = "img_activation_out"
            else:
        ...
        if branch_1d:
            ...
            if self.use_seq_encoder:
                ...
                if self.seq_enc_arch == "cnn":
                    (
                        ...
                        seq_conv2,
                        seq_conv1,
                    ) = self.conv_seq_unet_encoder(seq_input, seq_start_filters)
                    if not self.fullconv_seq_clf_length in [0, None]:
                        seq_final_pool = self.seq_fullconv_clf_linker(seq_drop5)
                        seq_final_pool = GlobalAveragePooling1D(name="final_1d")(
                            seq_final_pool
                        )  # latent space vector
                    ...
                    seq_drop5 = self.conv_block_seq_encoder(
                        seq_input, 128, 1, spatial_dropout=True
                    )
                    ...
                    seq_final_pool = self.trans_seq_encoder(
                        seq_input, input_seqlength, 5
                    )
            ...
            if self.use_seq_decoder:
                if self.seq_dec_arch == "cnn":
                    seq_activation_out = self.conv_seq_unet_decoder(
                        seq_drop5,
                        seq_drop4,
                        seq_conv3,
                        seq_conv2,
            ...
            if branch_1d and branch_2d:
                if self.merge_type == "concatenate":
                    merge = concatenate(
                        [
                            final_pool,
                            GlobalAveragePooling1D(name="final_1d")(seq_final_pool),
                        ],
                        axis=1,
                        name="merge_seq_img",
                    )
                    ...
                    img_shape = (
                        final_2d._keras_shape[1] * final_2d._keras_shape[2],
                        final_2d._keras_shape[3],
                    )
                ...
                if self.classifier == "dense":
                    final = GlobalAveragePooling1D(name="final")(
                        self.add_fclayers_to_model(seq_final_pool)
                    )
                elif self.classifier == "conv":
        ...
        if type(self.label_columns) == list:
            if self.dataset == "cath":
                outputs = self.add_cath_outputs_to_model(
                    final,
                    img_decoder=img_activation_out,
                    seq_decoder=seq_activation_out,
                )
            else:
                outputs = [Dense(1, name="dummy")(final)]
        else:
            outputs = self.add_outputs_to_model(
                final,
                unet_decoder=img_activation_out,
                seqnet_decoder=seq_activation_out,
            )
    ...
    def add_fclayers_to_model(self, x):
        ...
        if self.fc1 > 0:
            x = self.dense_block(
                x, self.fc1, 1, layer_order=self.batch_norm_order, names=["fc_first"]
            )
            x = Dropout(self.dropout)(x)
        if self.fc2 > 0:
            x = self.dense_block(
                x, self.fc2, 1, layer_order=self.batch_norm_order, names=["fc_last"]
            )
            x = Dropout(self.dropout)(x)
    ...
    def add_outputs_to_model(
        self, x, maxpool2D=None, unet_decoder=None, seqnet_decoder=None
    ):  # deprecated
        ...
        if type(x) != type(None):
            ...
            if 6 in self.label_columns:
                S35out = Dense(self.nb_classes[4], activation="softmax", name="S35out")(
                    x
                )
                outputs.append(S35out)
            if 7 in self.label_columns:
                S60out = Dense(self.nb_classes[5], activation="softmax", name="S60out")(
                    x
                )
                outputs.append(S60out)
            if 8 in self.label_columns:
                S95out = Dense(self.nb_classes[6], activation="softmax", name="S95out")(
                    x
                )
                outputs.append(S95out)
            if 9 in self.label_columns:
                S100out = Dense(
                    self.nb_classes[7], activation="softmax", name="S100out"
                )(x)
        ...
        if type(x) != type(None):
            ...
            for l in self.label_columns:
                if l in [
                    ...
                    outputs.append(
                        Dense(self.nb_classes[l], activation="softmax", name=l)(x)
                    )
    ...
    def get_callbacks(self, orig_model=None):
        ...
        if self.checkpoints > 0:
            chk = ModelCheckpointMultiGPU(
                os.path.join(
                    self.outdir,
                    self.modlabel + "_checkpoint.{epoch:02d}-{val_loss:.2f}.hdf5",
                ),
                orig_model,
                monitor="val_loss",
                verbose=0,
                ...
                mode="auto",
                period=self.checkpoints,
            )
            callbacks.append(chk)

        csvlog = CSVLogger(
            os.path.join(self.outdir, self.modlabel + "_csvlogger.csv"),
            separator=",",
            append=False,
        )
    ...
    def get_optimizer(self):
        ...
        if self.optimizer == "sgd":
            if self.learning_rate == -1:
                ...
                opt = optimizers.SGD(
                    lr=self.learning_rate, momentum=0.01, decay=0.01, nesterov=False
                )
    ...
    def get_train_val_dicts(self, input_dict, output_dict):
        ...
        split = train_test_split(
            *(
                [input_dict[i] for i in input_dict.keys()]
                + [output_dict[o] for o in output_dict.keys()]
            )
        )
        all_keys = []
        ...
        if False:
            if not self.use_img_decoder:
                if self.use_seq_encoder and self.use_img_encoder:
                    split = train_test_split(
                        input_dict["main_input"],
                        input_dict["seq_input"],
                        output_dict["cath01_class"],
                        output_dict["cath02_architecture"],
                        output_dict["cath03_topology"],
                        output_dict["cath04_homologous_superfamily"],
                        output_dict["sequence_length"],
                        test_size=self.valsplit,
                    )
                    train_input_dict = {"main_input": split[0], "seq_input": split[2]}
                    val_input_dict = {"main_input": split[1], "seq_input": split[3]}

                    train_output_dict = {
                        "cath01_class": split[4],
                        "cath02_architecture": split[6],
                        "cath03_topology": split[8],
                        "cath04_homologous_superfamily": split[10],
                        "sequence_length": split[12],
                    }
                    val_output_dict = {
                        "cath01_class": split[5],
                        "cath02_architecture": split[7],
                        "cath03_topology": split[9],
                        "cath04_homologous_superfamily": split[11],
                        "sequence_length": split[13],
                    }
                    ...
                    train_output_dict = {
                        "cath01_class": split[2],
                        "cath02_architecture": split[4],
                        "cath03_topology": split[6],
                        "cath04_homologous_superfamily": split[8],
                        "sequence_length": split[10],
                    }
                    val_output_dict = {
                        "cath01_class": split[3],
                        "cath02_architecture": split[5],
                        "cath03_topology": split[7],
                        "cath04_homologous_superfamily": split[9],
                        "sequence_length": split[11],
                    }
                ...
                if self.no_classifier:
                    ...
                    if self.use_seq_encoder and self.use_img_encoder:
                        ...
                        val_output_dict = {
                            ...
                            "cath04_homologous_superfamily": split[11],
                            "sequence_length": split[13],
                            "img_decoder": split[15],
                        }
                    elif (
                        self.use_seq_encoder
                        and not self.use_img_encoder
                        and self.no_classifier
                    ):
                        split = train_test_split(
                            input_dict["seq_input"],
                            output_dict["img_decoder"],
    ...
    def kfold_train_cath(
        ...
        if mode == "StratifiedKFold":
            ...
            splitter = StratifiedShuffleSplit(
                n_splits=n_splits, test_size=self.valsplit, random_state=random_state
            )
        ...
        for train_index, val_index in splitter.split(
            range(len(self.labels)), self.Ydict[target_columns], groups=groups
        ):
            df_train_indices = self.act_df.iloc[train_index,].index
            df_val_indices = self.act_df.iloc[val_index,].index

            self.train_cath(
                generic_label="F%i_" % count,
                train_indices=df_train_indices,
                val_indices=df_val_indices,
            )
        ...
        for i in range(n_splits):
            ...
            print(
                "Fold %i >>> loss: %d.4; val_loss: %d.4 --- %s"
                % (i + 1, h.history["loss"][-1], h.history["val_loss"][-1], m)
            )

    def train(
        self, generic_label="test", load_model=False, show_classnames=False
    ):  # deprecated
        if self.model == "none":
        ...
        if self.ngpus == 1:
            with open(
                os.path.join(self.outdir, "%s_model_history.json" % self.modlabel), "w"
            ) as hist:
                hist.write(str(self.history.history))
            self.keras_model.save_weights(self.model_weights_file)
            with open(self.model_arch_file, "w") as json:
    ...
    def train_cath(self, generic_label="test", load_model=False, show_classnames=False):
        ...
        (
            ...
            train_output_dict,
            val_output_dict,
        ) = self.get_train_val_dicts(input_dict, output_dict)
        ...
        self.history = self.keras_model.fit(
            ...
            verbose=self.train_verbose,
            callbacks=self.get_callbacks(
                orig_model=self.orig_model if self.ngpus > 1 else self.keras_model
            ),
        )
    ...
    def generator_train_h5(
        ...
        if self.img_bin_batch_sizes == []:
            if False:  # self.img_dim==None:
                ...
                if train_indices == None or val_indices == None:
                    ...
                    train_indices = all_indices[
                        : int(math.ceil(len(all_indices) * (1.0 - self.valsplit)))
                    ]  #  [int(i) for i in range(self.X.shape[0]) if i%2==0 ]
                ...
                self.history = self.keras_model.fit_generator(
                    ...
                    steps_per_epoch=max(1, self.crops_per_image)
                    * math.ceil(len(train_indices) / self.batch_size),
                    epochs=self.epochs,
                    verbose=self.train_verbose,
                    callbacks=callbacks,
                    validation_data=validation_generator,
                    validation_steps=max(1, self.crops_per_image)
                    * math.ceil(len(val_indices) / self.batch_size),
                    max_queue_size=20,
                    workers=24,
            ...
            s_largest_set_last, _, s_batch_sizes = zip(
                *sorted(
                    zip(self.img_size_bins, self.sn, self.img_bin_batch_sizes),
                    reverse=False,
                    key=lambda x: x[1],
                )
            )
            ...
            for si in range(len(s_largest_set_last)):
                ...
                output_labels = {
                    my_outputs[o]: self.dYmats[s][o] for o in range(len(my_outputs))
                }
                print(s)
    ...
    def plot_curves(self, metrics=["loss"]):
        ...
        for train_metric in metrics:
            ...
            if d[train_label][-1] != 0:
                plt.annotate(
                    str(round_figures(d[train_label][-1], 3)) + "(train)",
                    (actual_ep, d[train_label][-1]),
                    xycoords="data",
                    color=col,
                )
            if test_label != "":
                if d[test_label][-1] != 0:
                    plt.annotate(
                        str(round_figures(d[test_label][-1], 3)) + "(test)",
                        (actual_ep, d[test_label][-1]),
                        xycoords="data",
                        color=col,
                    )
            ...
            if test_label != "":
                all_test[train_metric] = d["val_" + train_metric]
            counter += 1

        if rf_val != None:
            plt.plot(
                [rf_val for i in range(self.epochs)],
                linestyle="--",
                color="black",
                label=None,
            )
            plt.annotate(
                str(round_figures(rf_val, 3)) + "(RF)",
                (actual_ep, rf_val),
                xycoords="data",
                color="black",
            )
        ...
        save_file = os.path.join(
            ...
            self.modlabel + "_" + metrics[0] + "_training-curve.png",
        )
    ...
    def plot_sample_images(self, indices=[]):
        ...
        if len(indices) == 0:
            ...
            indices = indices[:4]
        test_imgs = [
            np.squeeze(self.X[i]) if self.nchannels == 1 else self.X[i] for i in indices
        ]
        titles = [self.labels[i] for i in indices]
        # test_img = X[0,:,:,:]
        print("random test images:", test_imgs[0].shape)
        ...
        # plt.show()
        print(
            "writing: ", os.path.join(self.outdir, self.modlabel + "_random_images.png")
        )
    ...
    def visualize_image_data(
        ...
        if use_pfp:
            if self.verbose:
                print("Generating protein fingerprints...")
            from ProtConv2D.predictChembl import get_pfp_generator_model

            pfp_generator = get_pfp_generator_model(
                self.model_arch_file, self.model_weights_file, pfp_layername
            )
            if self.use_seq_encoder:
        ...
        method = "barnes_hut"
        assert method in ["barnes_hut", "exact"]
        metric = "cosine"
        assert metric in [
            "braycurtis",
            "canberra",
        ...
        if use_pixels:
            ...
            plt.suptitle(
                "input: protein image pixels of dim %i\nt-SNE with perplexity %.3f"
                % (self.img_dim, perp)
            )
            ...
            for l in self.Ys[:4]:
                ...
                if do_cluster_analysis:
                    if clustering_method == "hdbscan":
                        ...
                        clu = hdbscan.HDBSCAN(
                            metric="euclidean", min_cluster_size=self.nb_classes[count]
                        )
                ...
                if ndims == 2:
                    ...
                    # ax = plt.axes()
                    cax = ax.scatter(
                        x1, x2, s=marker_size, c=c, lw=0.5
                    )  # ,cmap=colmap)
                elif ndims == 3:
                    ax = fig.add_subplot(2, 2, count + 1, projection="3d")
                    cax = ax.scatter(x1, x2, x3, s=marker_size, c=c, lw=0.5)
                    ax.set_zlabel("ax3")
                ax.set_xlabel("x1")
                ax.set_ylabel("x2")
                ax.set_title(
                    ...
                    % (
                        names[count],
                        self.nb_classes[count],
                        " score=%s" % str(score) if score >= 0 else "",
                    )
                )
                # ax.set_title(title)
            ...
            files_written.append(
                os.path.join(save_path, self.modlabel + "_dataviz_pixels.png")
            )
            df = pd.DataFrame(
                {
                    "tsne1": x1,
            ...
            if 11 in self.label_columns:
                domlen = self.Ys[-2]
                cax = plt.scatter(
                    x1, x2, s=marker_size, c=domlen, lw=0.5
                )  # ,cmap=colmap)
                plt.xlabel("x1")
                plt.ylabel("x2")
                ...
                plt.savefig(
                    os.path.join(
                        save_path, self.modlabel + "_dataviz_pixels_domain_length.png"
                    )
                )
                files_written.append(
                    os.path.join(
            ...
            df.to_csv(
                os.path.join(save_path, self.modlabel + "_dataviz_pixels.csv"),
                index_label="cath_id",
            )
            files_written.append(
                os.path.join(save_path, self.modlabel + "_dataviz_pixels.csv")
        ...
        if use_pfp:
            ...
            fig = plt.figure(figsize=(20, 20))
            plt.suptitle(
                "input: protein fingerprints of length %i\nt-SNE with perplexity %.3f"
                % (self.fc2, perp)
            )
            clusters = []
            for l in self.Ys[:4]:
            ...
            if show_plots:
                plt.show()
            plt.savefig(
                os.path.join(save_path, self.modlabel + "_dataviz_pfprints.png")
            )
            files_written.append(
                os.path.join(save_path, self.modlabel + "_dataviz_pfprints.png")
            )
            ...
            if 11 in self.label_columns:
                ...
                domlen = self.Ys[-2]
                plt.scatter(x1, x2, s=marker_size, c=domlen, lw=0.5)  # ,cmap=colmap)
                plt.xlabel("x1")
                plt.ylabel("x2")
                plt.colorbar()
                plt.title(
                ...
                plt.savefig(
                    os.path.join(
                        save_path, self.modlabel + "_dataviz_pfprints_domain_length.png"
                    )
                )
                files_written.append(
                    os.path.join(
            ...
            if len(clusters) != 0:
                ...
                df["H_clusters"] = clusters[3]
            df.to_csv(
                os.path.join(save_path, self.modlabel + "_dataviz_pfprints.csv"),
                index_label="cath_id",
            )
            files_written.append(
                os.path.join(save_path, self.modlabel + "_dataviz_pfprints.csv")
    ...
    def get_pfp_generator_model(
        self, model_arch_file, model_weights_file, output_layer
    ):
        ...
        if type(my_layer) == type(None):
            print(
                "ERROR: could not find this layer to create a PFP generator:",
                output_layer,
            )
        if len(my_layer.output_shape) == 5:
            return Model(
                inputs=model.input, outputs=GlobalAveragePooling3D()(my_layer.output)
            )
        elif len(my_layer.output_shape) == 4:
            return Model(
                inputs=model.input, outputs=GlobalAveragePooling2D()(my_layer.output)
            )
        elif len(my_layer.output_shape) == 3:
            return Model(
                inputs=model.input, outputs=GlobalAveragePooling1D()(my_layer.output)
            )
        else:
            return Model(inputs=model.input, outputs=my_layer.output)

    def visualize_cath_image_data(
        self,
        show_plots=False,
        use_pixels=False,
        use_pfp=True,
        ...
        if use_pfp:
            ...
            pfp_generator = self.get_pfp_generator_model(
                self.model_arch_file, self.model_weights_file, pfp_layername
            )
            ...
            for lk in list(self.nb_classes.keys())[:4]:
                ...
                if do_cluster_analysis:
                    if clustering_method == "hdbscan":
                        ...
                        clu = hdbscan.HDBSCAN(
                            metric="euclidean", min_cluster_size=self.nb_classes[lk]
                        )
                    elif clustering_method == "kmeans":
            ...
            df = pd.DataFrame(
                {
                    "tsne1": x1,
                    "tsne2": x2,
                    "Class": self.Ydict["cath01_class"],
                    "Architecture": self.Ydict["cath02_architecture"],
                    "Topology": self.Ydict["cath03_topology"],
                    "Hom. Superfam.": self.Ydict["cath04_homologous_superfamily"],
                },
                columns=[
                    "Class",
                    "Architecture",
            ...
            df = pd.DataFrame(
                {
                    ...
                    "Architecture": self.Ydict["cath02_architecture"],
                    "Topology": self.Ydict["cath03_topology"],
                    "Homology": self.Ydict["cath04_homologous_superfamily"],
                },
                columns=[
                    "Class",
                    "Architecture",
    ...
    def plot_reconstruction_samples_seq2img(self):
        ...
        if self.ngpus == 1:
            input2d_model = Model(
                ...
                outputs=layer_dict["self_merge_lstm"].output,
            )
        else:
            input2d_model = Model(
                inputs=self.orig_model.input,
                outputs=layer_dict["self_merge_lstm"].output,
            )
        for i in range(0, 32, 6):
            ...
            print(d.shape)
            x = np.squeeze(self.X[i]).reshape(
                (1, self.img_dim, self.img_dim, self.nchannels)
            )
            print(x.shape)
            if self.no_classifier or len(self.keras_model.outputs) == 1:
                print("single output found")
                dec = self.keras_model.predict(
                    s.reshape((1, self.manual_seq_maxlength, seq_dim))
                )
            else:
                if "sequence_length" in self.label_columns:
                    C, A, T, H, L, dec = self.keras_model.predict(
                        s.reshape((1, self.manual_seq_maxlength, seq_dim))
                    )
                    print(C, A, T, H, L)
                else:
                    C, A, T, H, dec = self.keras_model.predict(
                        s.reshape((1, self.manual_seq_maxlength, seq_dim))
                    )
                    print(C, A, T, H)
            ...
            plt.title("matrix histogram: input")
            plt.show()
            # plt.colorbar(im)
            ...
            plt.subplot(8, 6, 3 + i)
            plt.imshow(
                x.astype("int").reshape(self.img_dim, self.img_dim, self.nchannels),
                aspect="equal",
            )
            ...
            plt.subplot(8, 6, 4 + i)
            sns.distplot(
                x.astype("int").flatten(), bins=256, kde=False, color="r"
            ).set_yscale("log")
            # histo.set_yscale('log');
            plt.xlabel("normalized values")
            plt.ylabel("log counts")
            plt.title("matrix histogram: ground truth")
            plt.show()

            # reconstruction
            plt.subplot(8, 6, 5 + i)
            plt.imshow(
                dec.astype("int").reshape(self.img_dim, self.img_dim, self.nchannels),
                aspect="equal",
            )
            ...
            plt.subplot(8, 6, 6 + i)
            sns.distplot(
                dec.astype("int").flatten(), bins=256, kde=False, color="r"
            ).set_yscale("log")
            # histo.set_yscale('log');
            plt.xlabel("normalized values")
            plt.ylabel("log counts")
            plt.title("matrix histogram: reconstruction")
            plt.show()
    ...
    def save_training_history(self):
        ...
        if self.verbose:
            print(results)
        results.to_csv(
            os.path.join(self.outdir, self.modlabel + "_results-history.csv"),
            index=False,
        )
