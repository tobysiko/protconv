...
from predictChembl import get_pfp_generator_model, add_protein_fingerprints
import ProtConv2D.keras_cath_classifier_v2 as cs

results_path = "/home-us/ts149092/results/models4pub2"
last_layer = "fc_last"
n_replicates = 5
...
for i in range(n_replicates):
	...
	for a in arch_names:
		print(a)
		if a in ["inception"]: dim = 299
		elif a in ["nasnetlarge"]: dim = 331
		else: dim = 224

		for l in fc_lengths:

			print(l)
			model_arch_file = glob.glob(os.path.join(results_path,"0%i"%(i+1),"kcc_%s*fc0x%s*_model_arch.json"%(a,l)))[0]
			model_weights_file = glob.glob(os.path.join(results_path,"0%i"%(i+1),"kcc_%s*fc0x%s*_model_weights.hdf5"%(a,l)))[0]
			model_history = glob.glob(os.path.join(results_path,"0%i"%(i+1),"kcc_%s*fc0x%s*_model_history.json"%(a,l)))[0]
			...
			clf = cs.CATH_Classifier(root_path, image_folder="pngbp", 
			                         ...
			                         model=a, data_labels_filename="cath-domain-list.txt", 
			                         ...
			                         valsplit=0.4, save_resized_images=False, outdir=results_path, 
			                         use_pretrained=True, early_stopping="val_loss", tensorboard=False, 
			                         optimizer="adam", verbose=False, batch_norm=True, act="relu",
			                         learning_rate=-1, img_size_bins=[], dropout=0.25, img_bin_batch_sizes=[], 
			                         flipdia_img=False, inverse_img=False, 
			                         fc1=0, fc2=int(l), keras_top=True ,ngpus=1, train_verbose=False,
			...
			scores = {}
			new_files = clf.visualize_image_data(use_pixels=False, use_pfp=True, save_path=os.path.join(results_path,"0%i"%(i+1)), do_cluster_analysis=True, clustering_method="kmeans", scores_dict=scores )
			print(i,a,l,n_epochs, train_loss, val_loss, train_C_acc, val_C_acc, train_A_acc, val_A_acc, train_T_acc, val_T_acc, train_H_acc, val_H_acc, 
			...
			results_table.append([a,l,n_epochs, train_loss, val_loss, train_C_acc, val_C_acc, train_A_acc, val_A_acc, train_T_acc, val_T_acc, train_H_acc, val_H_acc, 
				scores["pfprints_Class_homogeneity_score"],
				scores["pfprints_Architecture_homogeneity_score"],
				scores["pfprints_Topology_homogeneity_score"],
				scores["pfprints_Homology_homogeneity_score"]])
...
df = pd.DataFrame(results_table, columns=["arch","pfp_length","n_epochs", "train_loss", "val_loss", "train_C_acc", "val_C_acc", 
	"train_A_acc", "val_A_acc", "train_T_acc", "val_T_acc", "train_H_acc", "val_H_acc",
	"pfprints_Class_homogeneity_score","pfprints_Architecture_homogeneity_score","pfprints_Topology_homogeneity_score","pfprints_Homology_homogeneity_score"])
