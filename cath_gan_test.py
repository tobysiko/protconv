...
for p in params:
    clf = cs.CATH_Classifier(
                    root_path, 
                    img_dim=dim, batch_size=bs, epochs=2000, 
                    ...
                    valsplit=0.1, save_resized_images=False, outdir=results_dir, 
                    use_pretrained=True,
                    ...
                    generic_label="seq2U-GAN", h5_backend="h5py",
                    
                    use_img_encoder=False, use_img_decoder=True,
                    cath_loss_weight=1.0, dc_decoder_loss="mean_absolute_error",checkpoints=20,
                    cath_domlength_weight=0.1, domlength_regression_loss="mean_absolute_error",
                    blank_img_frac=0.0,
                    
                    use_seq_encoder=True, use_seq_decoder=False, 
                    seq_dec_weight=1.0, manual_seq_maxlength=1024,
    ...
    try:
        new_files =clf.visualize_cath_image_data(
                ...
                use_pixels=False, 
                use_pfp=True, 
                pfp_layername="global_average_pooling2d_1" if not clf.no_classifier else 'dropout_2', 
                ndims=2, 
