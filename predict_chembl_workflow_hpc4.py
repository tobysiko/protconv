...
from keras.layers import Input, Dense, Reshape, Dropout, Activation, Flatten, MaxPooling1D,  GlobalAveragePooling1D, AveragePooling1D, Conv1D, merge
from keras.layers.merge import add, multiply, dot, subtract, average, concatenate, maximum
from keras.layers.normalization import BatchNormalization
from keras.utils import to_categorical

class My_Keras_Model(Model):


    def __init__(self, is_classifier, feat1, feat2, dropout_rate):
        self.is_classifier = is_classifier
        self.feat1 = feat1
        self.feat2 = feat2
        self.dropout_rate = dropout_rate

    def set_feat1(self, val):
        self.feat1=val
        self.create_model()

    def set_feat2(self, val):
        self.feat2 = val
        self.create_model()

    def create_model(self):
        input1 = Input((self.feat1, 1), dtype='float32', name='input1')
        input2 = Input((self.feat2, 1), dtype='float32', name='input2')
    
        x1 = Conv1D(16, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(input1) ) )
        x1 = Conv1D(16, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x1) ) )
        x1 = Dropout(self.dropout_rate)(x1)
        x1 = MaxPooling1D(2)(x1)

        x1 = Conv1D(32, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x1) ) )
        x1 = Conv1D(32, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x1)) )
        x1 = Dropout(self.dropout_rate)(x1)
        x1 = AveragePooling1D(2)(x1)

        x2 = Conv1D(16, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(input2) ) )
        x2 = Conv1D(16, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x2)) )
        x2 = Dropout(self.dropout_rate)(x2)
        x2 = MaxPooling1D(2)(x2)

        x2 = Conv1D(32, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x2) ) )
        x2 = Conv1D(32, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x2)) )
        x2 = Dropout(self.dropout_rate)(x2)
        x2 = AveragePooling1D(2)(x2)

        merge = concatenate([x1, x2], axis=1)
        x = Conv1D(64, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(merge) ) )
        x = Conv1D(64, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Dropout(self.dropout_rate)(x)
        x = AveragePooling1D(2)(x)

        x = Conv1D(128, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Conv1D(128, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Dropout(self.dropout_rate)(x)
        x = AveragePooling1D(2)(x)
    
        x = Conv1D(256, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Conv1D(256, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Dropout(self.dropout_rate)(x)
        x = AveragePooling1D(2)(x)

        x = Conv1D(512, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Conv1D(512, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Dropout(self.dropout_rate)(x)
        x = AveragePooling1D(2)(x)

        x = Conv1D(1024, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Conv1D(1024, 3, padding = 'same', kernel_initializer = 'he_normal' )( Activation('relu')( BatchNormalization()(x) ) )
        x = Dropout(self.dropout_rate)(x)
        x = AveragePooling1D(2)(x)
        ...
        self.model = Model(inputs=[input1, input2], outputs=[ final ])

def build_keras_model1(is_classifier=True, n_feat=512, dropout_rate=0.5):
    model = Sequential()
    
    model.add(Dense(n_feat, input_dim=n_feat))
    model.add( BatchNormalization() )
    model.add( Activation('relu') )
    model.add( Dropout( dropout_rate) )

    model.add(Dense(n_feat/2))
    model.add( BatchNormalization() )
    model.add( Activation('relu') )
    model.add( Dropout( dropout_rate) )

    model.add(Dense(n_feat/4))
    model.add( BatchNormalization() )
    model.add( Activation('relu') )
    model.add( Dropout( dropout_rate) )
...
def build_keras_model2(is_classifier=True, n_feat=512, dropout_rate=0.5, n_blocks=3, start_filters=16):
    model = Sequential()
    ...
    for i in range(n_blocks):
        if i==0:
            model.add( Reshape((n_feat,1,), input_shape=(n_feat,)))
            model.add( Conv1D(start_filters*(i+1)*2, 3, padding = 'same', kernel_initializer = 'he_normal' , input_shape=(n_feat,1,)) )
        else:
            model.add( Conv1D(start_filters*(i+1)*2, 3, padding = 'same', kernel_initializer = 'he_normal' ) )
            model.add( Conv1D(start_filters*(i+1)*2, 3, padding = 'same', kernel_initializer = 'he_normal' ) )
            model.add( BatchNormalization() )
            model.add( Activation('relu') )
            model.add( Dropout( dropout_rate) )
...
def build_keras_model3(is_classifier=True, feat1=512, feat2=512, dropout_rate=0.5):

    #model = My_Keras_Model(is_classifier=is_classifier, feat1=feat1, feat2=feat2, dropout_rate=dropout_rate)
    #model.create_model()
    input1 = Input(( feat1, 1), dtype='float32', name='input1')
    input2 = Input(( feat2, 1), dtype='float32', name='input2')
...
if __name__=="__main__":   
    ...
    data_loc = "/hpc/projects/ts149092/chembl/"#%(hpc_folder)
    ...
    biovec_fasta = "/hpc/projects/ts149092/cath/cath-domain-seqs.fa"#%(hpc_folder)
    ...
    predictor_type = sys.argv[3]
    assert predictor_type in ["RF", "ET", "GB", "DNN"]

    print("predictor_type", predictor_type)

    df = pd.read_csv(csv_filename)
    
    n_fixed_clusters = int(sys.argv[4]) #= fold cross validation = number k of clusters

    last_layer = "fc_last"
        
    if len(sys.argv)==6:
        last_layer = sys.argv[5]
    print("Name of layer for extracting fingerprints:",last_layer)
    ...
    if False:
        ...
        k2c = {}
        for k in krange:
            clu = KMeans(n_clusters=k, random_state=kmeans_random_seed)
            clu.fit(X_pfp_clu)
        ...
        X_pfp_clu = X_df.set_index("best_match").filter(regex="pfp").dropna().drop_duplicates()
        print( X_pfp_clu.shape)
        clu = KMeans(n_clusters=n_fixed_clusters, random_state=kmeans_random_seed)
        clu.fit(X_pfp_clu)
        score = silhouette_score(X_pfp_clu, clu.labels_, metric="euclidean")
        print(score)
    ...
    if True:
        if False:
            ...
            k2c = {}
            for k in krange:
                clu = KMeans(n_clusters=k, n_init = 20, n_jobs=n_cpus, random_state=kmeans_random_seed)
                clu.fit(X_cfp_clu)
    ...
    Xc_cmpd_pfp_protvec = X_df[X_df["pchembl_class"]!=1].merge(seq_df, how="left", left_on="best_match", right_index=True, validate="many_to_one"  ).filter(regex=r"(cfp|pfp|pv)[0-9]*")
    Xr_cmpd_pfp_protvec = X_df.merge(seq_df, how="left", left_on="best_match", right_index=True, validate="many_to_one"  ).filter(regex=r"(cfp|pfp|pv)[0-9]*")
    print ("cmpd_pfp_protvec",Xc_cmpd_pfp_protvec.shape, Xr_cmpd_pfp_protvec.shape)

    yc = X_df[X_df["pchembl_class"]!=1].loc[:,"pchembl_class"]
    yc[yc==2] = 1
    yc_dnn=yc#to_categorical(yc)
    ...
    if predictor_type=="DNN":
        clf = None
        parameters_clf = { "dropout_rate":(0.25,0.5) }
        reg = None
        parameters_reg = { "dropout_rate":(0.25,0.5) }
    elif predictor_type=="RF":
        clf = RandomForestClassifier(oob_score=True)
        parameters_clf = { "max_depth":(None,10,100),  "max_features":(None,'sqrt'), "n_estimators":(100, 1000) }
        reg = RandomForestRegressor(oob_score=True)
        parameters_reg = { "max_depth":(None,10,100),  "max_features":(None,'sqrt'), "n_estimators":(100, 1000)}
    ...
    #TODO; https://github.com/capkuro/Keras-GridSearchCV
    # how to run grid search on multiple GPUs (ideally across multiple nodes) ?
    ...
    for X_name in sorted(X_dict.keys()):
        ...
        for cv in cv_list:
            ...
            if X_name[:2]=="Xc":
                ...
                if predictor_type=="DNN":
                    clf = KerasClassifier(build_fn=build_keras_model1, is_classifier=True, n_feat=len(c_features)+len(p_features), 
                                            verbose=0, epochs=100, validation_split=0.4, shuffle=1)#, callbacks=callbacks) # Sequential.fit kwargs
                    n_cpus=1
                    
                gs = GridSearchCV(clf, parameters_clf, cv=this_cv, scoring=clf_scorers, refit=clf_refit, n_jobs=n_cpus)
                if predictor_type=="DNN":
                    gs.fit(X=X_dict[X_name], y=yc_dnn, groups=grp)
                else:
                ...
                if predictor_type=="DNN":
                    reg = KerasRegressor(build_fn=build_keras_model1, is_classifier=False, n_feat=len(c_features)+len(p_features), n_blocks=3, start_filters=16, 
                                            verbose=0, epochs=100, validation_split=0.4, shuffle=1)#, callbacks=callbacks) # Sequential.fit kwargs
                    n_cpus=1
            ...
            if predictor != "":
                ...
                df["c_len"] = len(X_dict[X_name].columns[:c_len]) 
                df["p_len"] = len(X_dict[X_name].columns[c_len:])
                if predictor_type!="DNN":
